...
sequenceDiagram
  ...
  API->>Run: runChat(history, step_context)
  Run->>Run: system = persona(cached) + clock + scenario + recent push
  loop up to 6 times
    Run->>LLM: messages.create({system, tools[24], messages})
    alt stop_reason = tool_use
      LLM-->>Run: tool_use blocks
      par each block
        Run->>Run: suggest_action? → allow-list(12) → actions[]
        Run->>Tools: dispatchTool(name, input)
        Tools->>DB: select … lte(asOf)
        DB-->>Tools: rows
        Tools-->>Run: JSON (or {error})
      end
      Run->>Run: messages += assistant tool_use + user tool_result
    else stop_reason = end_turn
      LLM-->>Run: text
      Run->>Run: RM gate: gated action ∧ (completion claim ∨ boilerplate) → template text
    end
  end
  Run-->>API: {reply, tool_calls[], actions[], stop_reason}
